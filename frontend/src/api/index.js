...
const createApi = (base) => {
    const getApiPath = (path) => {
        return `${base}${base.endsWith('/') ? '' : '/'}${path.startsWith('/') ? path.slice(1) : path}`;
    };
...
export const dataApi = createApi(config.dataApi);
export const jazzApi = createApi(config.jazzApi);
